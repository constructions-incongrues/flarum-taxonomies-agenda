...
export default class EventComposer extends DiscussionComposer {
  ...
  oninit(vnode: any) {
    ...
    if (this.composer && this.composer.fields) {
      ...
      const titleToSet = a.editingTitle || a.prefillTitle;
      if (titleToSet && typeof this.title === 'function') {
  ...
  headerItems() {
    ...
    const t = (k: string) => app.translator.trans('taxonomies-agenda.forum.composer.' + k);
    const titleValue = typeof this.title === 'function' ? this.title() : '';
    const busy = this.loading || this.composer?.loading;
    const editingId = (this.attrs as EventPrefillAttrs).editingId;
    ...
    items.add('event-details', (
      <div className="EventComposer-details">
        <header className="EventComposer-brand">
          ...
          <span>{editingId ? t('edit_brand_label') : t('brand_label')}</span>
        </header>
  ...
  onsubmit() {
    ...
    if (editingId) {
      ...
      app.request({
        method: 'PATCH',
        url: app.forum.attribute('apiUrl') + '/discussions/' + editingId,
        body: {
          data: {
            type: 'discussions',
            id: String(editingId),
            attributes: { title, taxonomies: d.taxonomies },
          },
        },
      }).then(
        () => {
